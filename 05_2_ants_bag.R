...
#' So one run of 5-fold CV with `cv_ants` takes a little over 2 seconds on my
#' laptop. Thus, 250 reps using the `for` loop above would take about 9 minutes:
...
# save(cv_error, file="05_2_ants_bag_files/saved/cv_error.Rdata")
load("05_2_ants_bag_files/saved/cv_error.Rdata")

#' That completed in 2 minutes, about a 3.5X speedup. A rough rule of thumb is
#' that you can expect a maximum speedup of somewhat less than n * X, where n is
#' the number of hardware cores on your computer. My laptop has 16 cores. Some
#' computers also have multiple virtual cores for each hardware core. To find
#' out how many cores your computer has:
...
#' make full use of the hardware you might need to use all the virtual cores. On
#' some systems, maximum performance is achieved when the number of workers
#' matches the number of hardware cores, and for some algorithms when the number
#' of workers exceeds the number of virtual cores. Further information on the
#' `future` implementation of parallel processing is available in [Bengtsson
#' 2021](https://journal.r-project.org/archive/2021/RJ-2021-048/index.html). See
#' also the package vignettes for `doFuture` and `doRNG`.
#' 
...
# save(cv_error_br, boot_reps, file="05_2_ants_bag_files/saved/br_experiment.Rdata")
load("05_2_ants_bag_files/saved/br_experiment.Rdata")
