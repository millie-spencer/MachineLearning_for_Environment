...
#' model formula syntax to train the model by minimizing the SSQ (sum of squares) with the
#' function `lm`.
...
# yields identical results as above 
...
# raw=TRUE → gives you the regular (non-orthogonal) polynomials
# raw=FALSE (the default) → gives you the orthogonal polynomials
...
#' remove the correlation among the polynomial terms (latitude). Orthogonal polynomials are
...
#' Test the function
cv_poly_ants(forest_ants, k=10, order=4) # yields 17.10... 
cv_poly_ants(forest_ants, k=22, order=4) # yields 15.51... 
